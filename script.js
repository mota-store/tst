...
(function () {
  ...
  let quizStartTime = 0;
  let nameConfirmPending = false;

  // Número de "cotação" cosmético no cartão, só para reforçar a sensação
  // de carteirinha/documento real. Não tem função de rastreamento.
  const quoteSeqEl = document.getElementById('quoteSeq');
  if (quoteSeqEl) {
    const n = Math.floor(1000 + Math.random() * 8999);
    quoteSeqEl.textContent = new Date().getFullYear() + '-' + n;
  }
  ...
  function isLikelySpam() {
    const honeypot = $('#inputWebsite');
    if (honeypot && honeypot.value.trim() !== '') return true; // campo-armadilha preenchido = bot
    const elapsed = Date.now() - quizStartTime;
    if (quizStartTime && elapsed < 3500) return true; // rápido demais para uma pessoa real
    return false;
  }
  ...
  const warningNome = $('#warningNome');
  const btnQ1 = $('#btnQ1');

  const NOME_BLACKLIST = [
    'abacate', 'banana', 'laranja', 'morango', 'teste', 'testando',
    'fulano', 'ciclano', 'beltrano', 'anonimo', 'anônimo', 'desconhecido',
    'asdf', 'qwerty', 'nao sei', 'não sei', 'xxx', 'kkkk', 'kkkkk'
  ];

  function isSuspiciousName(value) {
    const v = normalize(value);
    if (!v) return false;
    if (NOME_BLACKLIST.includes(v)) return true;
    if (/\d/.test(v)) return true;                 // contém números
    if (/(.)\1{3,}/.test(v)) return true;           // letra repetida (ex: kkkk, aaaa)
    if (!/[aeiou]/.test(v)) return true;            // nenhuma vogal
    if (v.replace(/[^a-z]/g, '').length < 3) return true; // curto demais
    return false;
  }

  function resetNomeWarning() {
    nameConfirmPending = false;
    warningNome.classList.remove('is-visible');
    btnQ1.textContent = 'Continuar';
  }
  ...
  $('[data-next="q1"]').addEventListener('click', () => {
    ...
    if (isSuspiciousName(value) && !nameConfirmPending) {
      warningNome.classList.add('is-visible');
      warningNome.classList.remove('is-shaking');
      void warningNome.offsetWidth;
      warningNome.classList.add('is-shaking');
      btnQ1.textContent = 'Continuar mesmo assim';
      nameConfirmPending = true;
      return;
    }
  ...
  function handleFinalScreen() {
    ...
    if (isLikelySpam()) {
      // Não redireciona sozinho; deixa só a ação manual, sem alarde.
      finalSubtitle.innerHTML = 'Recebemos suas respostas.<br>Toque abaixo para receber sua cotação personalizada.';
      return;
    }

    finalSubtitle.innerHTML = 'Recebemos suas respostas.<br>Você já vai ser redirecionado(a) para o WhatsApp...';
    setTimeout(goToWhatsApp, 1800);
  }

  whatsappBtn.addEventListener('click', goToWhatsApp);
