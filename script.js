...
(function () {
   ...
   * Configuração
   * ------------------------------------------------------------------ */
  const WHATSAPP_NUMBER = '559198307032'; // +55 91 98307-032

  // Ordem das telas do fluxo (sem contar welcome/loading/final)
  const QUESTION_SCREENS = ['q1', 'q2', 'q3', 'q4', 'q5', 'q6', 'q7', 'q8'];
  ...
  const idadeOptions = $('#idadeOptions');
  FAIXAS_IDADE.forEach(faixa => {
    const chip = document.createElement('button');
    chip.type = 'button';
    chip.className = 'option-chip';
    chip.textContent = faixa;
    chip.addEventListener('click', () => {
      $all('.option-chip', idadeOptions).forEach(c => c.classList.remove('is-selected'));
      chip.classList.add('is-selected');
      answers.idade = faixa;
      clearError('#idadeOptions', 'errorIdade');
      setTimeout(() => advanceFrom('q4'), 260);
    });
    idadeOptions.appendChild(chip);
  });
